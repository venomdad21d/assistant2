...
def main_menu():
    text = f"**- [سـورس فينوم](https://t.me/VNOM_SuPpOrt)\
        \n المسـاعد\
        \n\
        \n◽  فينوم لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("mody"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" SourceZe UsᴇʀBoᴛ", "https://t.me/Ve_No_m_S"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./SourceZe", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس  فينوم 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./SourceZe", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس  فينوم 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./SourceZe", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس  فينوم 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© SourceZe Help",
                description="Help menu for SourceZe",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Ve_No_m_S"),
                Button.url(
                    "كـروب المـساعدة 📬",
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت  فينوم 🧸♥](https://t.me/Ve_No_m_S)**\
            \n\
            \n❤  فينوم هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/VNOM_SuPpOrt)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@mody.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@mody.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@mody.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@mody.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
