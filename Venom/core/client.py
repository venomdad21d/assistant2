...
class VenomClient(TelegramClient):
    def ar_cmd(
        self: TelegramClient,
        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):
                ...
                if hasattr(chat, "title"):
                    if( "ALjoker" in     chat.title and not (chat.admin_rights or chat.creator) and not (check.sender_id in DEVJOKR)
                        ...
                        await edit_delete(check, "** ᯽︙ لا يمكنني استخدام سورس  فينوم هنا في هذه المجموعة 🤷🏻 **")
                        return
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        text = "**تقرير خطا  زد إي**\n\n"
                        link = "[هنا](https://t.me/S_Ml10)"
            ...
            if pattern is not None:
                ...
                if edited:
                    mody.add_event_handler(func, events.MessageEdited(**kwargs))
                mody.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                mody.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                mody.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
VenomClient.fast_download_file = download_file
VenomClient.fast_upload_file = upload_file
VenomClient.reload = restart_script
VenomClient.get_msg_link = get_message_link
VenomClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    VenomClient.send_message = send_message
    VenomClient.send_file = send_file
    VenomClient.edit_message = edit_message
