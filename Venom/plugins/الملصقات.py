...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"SourceZe_{userid}_{pack}_anim"
    elif is_video:
        return f"SourceZe_{userid}_{pack}_vid"
    return f"SourceZe_{userid}_{pack}"
...
@mody.on(admin_cmd(pattern="حزمة"))
async def SourceZepkg(_):
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمة بنجاح ✓ \nالحزمة  → [اضغط هنا](https://t.me/addstickers/{SourceZe_Jep.set.short_name})**")

@mody.on(admin_cmd(pattern="حزمه"))
async def SourceZepkg(_):
    Jep = await _.get_reply_message()
    if not Jep:
        return await edit_or_reply(_, "**- يجب عليك الرد على حزمة.**")
