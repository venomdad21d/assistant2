...
@mody.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@mody.ar_cmd(pattern="توب الفلوس(.*)")
...
@mody.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@mody.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@mody.ar_cmd(func=lambda m:"راتب")
...
- مثال: .انشاء حساب زد إي او المسلم
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
async def thief(message):
    ...
    SourceZe = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await mody.send_file(
                message.chat_id,
                "https://telegra.ph/file/9c4007ca621cc01a3c650.jpg",
                caption=f"سرق [{SourceZe}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
@mody.ar_cmd(pattern="انشاء حساب (.*)")
