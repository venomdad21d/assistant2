...
@mody.on(admin_cmd(pattern="(جلب الصورة|جلب الصوره|ذاتيه|ذاتية)"))
...
- Dev: @ELHYBA 
  """,
    ...
    await event.delete()
#By @jepthon For You 🌹
@mody.on(admin_cmd(pattern="(الذاتية تشغيل|ذاتية تشغيل)"))
...
@mody.on(admin_cmd(pattern="(الذاتية تعطيل|ذاتية تعطيل)"))
...
@mody.on(events.NewMessage(func=lambda e: e.is_private and joker_unread_media(e) and e.sender_id != bot.uid))
async def Reda(event):
    if gvarstatus("savepicforme"):
        caption = """**
