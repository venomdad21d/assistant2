...
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, mody, logging
...
autopic_path = os.path.join(os.getcwd(), "SourceZe", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "SourceZe", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "SourceZe", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "SourceZe", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "SourceZe", "photo_pfp.png")
            ...
            if i > 0:
                await mody(
                    functions.photos.DeletePhotosRequest(
                        await mody.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            result = await mody(functions.photos.UploadProfilePhotoRequest(file=await mody.upload_file(autophoto_path)))
...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        try:
            if i > 0:
                async for photo in mody.iter_profile_photos(int(dgp), limit=1) :
                    await mody(
            ...
            i += 1
            await mody(functions.channels.EditPhotoRequest(int(dgp), file))
            os.remove(auto_group_photo_path)
            await asyncio.sleep(60)
        except ChatAdminRequiredError:
            return await mody.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
...
async def group_loop():
    ...
    while AUTONAMESTAR:
        ...
        try:
            await mody(functions.channels.EditTitleRequest(
                channel=await mody.get_entity(int(ag)),
                title=name
            ))
        except ChatAdminRequiredError:
            await mody.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
        except ChannelInvalidError:
            return
        except FloodWaitError:
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await mody(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@mody.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@mody.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@mody.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@mody.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@mody.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
mody.loop.create_task(digitalpicloop())
mody.loop.create_task(digitalgrouppicloop())
mody.loop.create_task(autoname_loop())
mody.loop.create_task(autobio_loop())
mody.loop.create_task(group_loop())
