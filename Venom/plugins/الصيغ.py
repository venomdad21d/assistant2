...
async def save_media(event):
    ...
    try:
        message = await mody.get_messages(int(channel_id), ids=int(message_id))
        if not message:
        ...
        if message.media or message.document:
            ...
            await mody.send_message(event.chat_id, file_ext)
            file_path = os.path.join(save_dir, f"media_{file_ext}")
            await mody.download_media(message, file=file_path)

            await mody.send_file('me', file=file_path, caption=message.text)
...
async def Hussein(event):
    ...
    try:
        channel_entity = await mody.get_entity(channel_username)
        messages = await mody.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await mody.send_file("me", file=file_path)
                os.remove(file_path)
