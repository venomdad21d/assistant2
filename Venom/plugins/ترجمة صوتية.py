...
import subprocess
from SourceZe.helpers.functions.functions import translate
...
@mody.ar_cmd(pattern="test(?:\s|$)([\s\S]*)")
async def reda(event):
    tr = translate("انا عراقي", lang_tgt="fa").replace("\ N", "\n")
    await edit_or_reply(event, tr)
    result = await mody(functions.users.GetFullUserRequest(
...
@mody.ar_cmd(pattern="تكلم(?:\s|$)([\s\S]*)")
...
async def _(event):
    ...
    SourceZeevent = await edit_or_reply(event, "⌔∮ جـار التسجيل انتـظر قليلا")
    ...
    try:
        ...
        try:
            ...
            await SourceZeevent.edit(str(exc))
        ...
        await edit_or_reply(SourceZeevent, f"خطأ:\n{e}")
