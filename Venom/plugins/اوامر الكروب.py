...
async def ban_user(chat_id, i, rights):
    try:
        await mody(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        
@mody.on(events.NewMessage(outgoing=True, pattern="ارسل?(.*)"))
...
@mody.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await mody(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in mody.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await mody.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@mody.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@mody.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@mody.ar_cmd(pattern="الغاء التفليش")
...
@mody.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await mody.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
async def Hussein(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await mody.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await mody(LeaveChannelRequest(channel_username))
...
@mody.ar_cmd(pattern="مغادرة القنوات")
async def Hussein (event):
...
@mody.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@mody.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
is_Reham = False
No_group_Joker = "@UI_OS"
# يا يلفاشل هم الك نيه تاخذه وتنشره بسورسك 🤣
active_aljoker = []

@mody.ar_cmd(pattern=r"الذكاء تفعيل")
...
@mody.ar_cmd(pattern=r"الذكاء تعطيل")
...
async def Hussein(event):
    ...
    if sender_id != 6581896306:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            aljoker_entity = await mody.get_entity(sender.id)
...
@mody.ar_cmd(pattern="المميز تفعيل")
...
@mody.ar_cmd(pattern="المميز تعطيل")
async def disable_emoji_blocker(event):
    global Ya_Hussein
    Ya_Hussein = False
