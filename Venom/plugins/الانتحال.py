...
#Reda
jeps = ["SourceZe", "SourceZesupport"]
@mody.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await mody(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await mody(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await mody(functions.channels.EditTitleRequest(
                channel=await mody.get_entity(event.chat_id),
        ...
        try:
            await mody(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await mody.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await mody.upload_file(profile_pic)
        try:
            await mody(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
...
#Reda
@mody.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in mody.iter_profile_photos(event.chat_id, limit=1) :
                    await mody(
