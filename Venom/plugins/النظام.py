...
async def psu(event):
    ...
    softw = "** 𓆩 𝑺𝑶𝑼𝑹𝑪𝑬 𝐙𝐄 𝑺𝒀𝑺𝑻𝑬𝑴 𝑰𝑵𝑭𝑶 𓆪 **\n"
...
async def cpu(event):
    ...
    cmd = "ar /proc/cpuinfo | grep 'model name'"
    o = (await _arutils.runcmd(cmd))[0]
    await edit_or_reply(
        event, f"**[SourceZe](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    arevent = await edit_or_reply(sysd, "`Fetching system information.`")
    cmd = "git clone https://github.com/dylanaraps/neofetch.git"
    await _arutils.runcmd(cmd)
    neo = "neofetch/neofetch --off --color_blocks off --bold off --cpu_temp C \
                    --cpu_speed on --cpu_cores physical --kernel_shorthand off --stdout"
    a, b, c, d = await _arutils.runcmd(neo)
    result = str(a) + str(b)
    await edit_or_reply(arevent, f"**Neofetch Result:** `{result}`")
