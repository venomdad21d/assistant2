...
from SourceZe import HEROKU_APP, UPSTREAM_REPO_URL, mody
...
UPSTREAM_REPO_BRANCH = "SourceZe"

REPO_REMOTE_NAME = "temponame"
IFFUCI_ACTIVE_BRANCH_NAME = "SourceZe"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/SourceZe"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/SourceZe", force=True)
    ...
    try:
        remote.push("master:SourceZe", force=True)
    except Exception as error:
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("SourceZe", origin.refs.SourceZe)
        repo.heads.SourceZe.set_tracking_branch(origin.refs.SourceZe)
        repo.heads.SourceZe.checkout(True)
    ...
    if conf == "الان":
        ...
        await event.edit("ᯓ 𝐒𝐎𝐔𝐑𝐂𝐄 • 𝐙𝐄 🔱  - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت زد إي .. انتظـر . . .🌐**\n\n%𝟷𝟶 ▬▭▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝐒𝐎𝐔𝐑𝐂𝐄 • 𝐙𝐄 🔱  - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت زد إي .. انتظـر . . .🌐**\n\n%𝟸𝟶 ▬▬▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝐒𝐎𝐔𝐑𝐂𝐄 • 𝐙𝐄 🔱  - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت زد إي .. انتظـر . . .🌐**\n\n%𝟹𝟶 ▬▬▬▭▭▭▭▭▭▭")
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "" and not force_update:
            ...
            return await event.respond(
                f"⌔ :  لتحديث سورس زد إي ارسل : `.تحديث الان` "
            )
    ...
    if event.reply_to and event.sender_id in progs:
        ...
        if owner_id == mody.uid:
            if event.message.message == "حدث":
                conf = "الان"
                event = await event.reply("**ᯓ 𝐒𝐎𝐔𝐑𝐂𝐄 • 𝐙𝐄 🔱  - تحـديثـات السـورس\n**•─────────────────•**\n\n**✾╎جـاري تـحــديــث الـسـورس بأمــر مـن المـطور ⎌****")
                off_repo = UPSTREAM_REPO_URL
                force_update = False
