...
@mody.ar_cmd(
    ...
    info={
        "header": "To list all plugins in SourceZe.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in SourceZe"
    cmd = "ls SourceZe/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[ زد إي](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@mody.ar_cmd(
    ...
    info={
        "header": "To list all environment values in SourceZe.",
        "description": "to show all heroku vars/Config values in your SourceZe",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in SourceZe"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[ زد إي](tg://need_update_for_some_feature/) قـائمـة الـفـارات:**\n\n\n{o}\n\n**انتبه هنالك معلومات حساسة لا تُعطِها لشخص غير موثوق**"
    )
    ...
    await mody.send_message("me", OUTPUT)
...
@mody.ar_cmd(pattern="رابط مباشر")
