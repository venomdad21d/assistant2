#    جميع الحقوق محفوظة كتابة وتعديل  :   @ELHYBA
...
from SourceZe import CMD_HELP, mody
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @Source_Ze", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@mody.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(SourceZe):
    dc = razan = SourceZe
    i = 0
    sender = await dc.get_sender()
    me = await SourceZe.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await SourceZe.get_chat()
    if SourceZe.is_private:
        user = SourceZe.chat
        rank = SourceZe.pattern_match.group(1)
    else:
        SourceZe.chat.title
    try:
        user, rank = await get_full_user(SourceZe)
    except:
        pass
    if me == user:
       mody = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await SourceZe.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await SourceZe.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@mody.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(SourceZe):
    dc = razan = SourceZe
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       mody = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
