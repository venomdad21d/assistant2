...
@mody.ar_cmd(pattern="اضف (.*)")
async def custom_SourceZe(event):
...
@mody.ar_cmd(pattern="اضف صورة (الفحص|فحص) ?(.*)")
...
@mody.ar_cmd(pattern="اضف صورة (البنك|بنك) ?(.*)")
async def add_ping_aljoker(event):
    ...
    if reply and reply.media:
        ...
        await event.edit("**۞︙ يرجى الرد على الصورة لتحديث الفار**")
@mody.ar_cmd(pattern="اضف صورة (الحماية|الحمايه|حماية|حمايه) ?(.*)")
